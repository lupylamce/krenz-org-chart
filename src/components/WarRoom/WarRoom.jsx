                  ...
                  {selectedAppDetail.status.includes('pending') && (
                    <div className="mt-4 pt-4 border-t border-gray-100 flex flex-col gap-3">
                      ...
                      <div className="flex gap-3">
                        <button onClick={() => { handleApproveApplication(selectedAppDetail, 'reject'); setSelectedAppDetail(null); }} className="flex-1 py-3 bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 rounded-xl font-black transition text-sm">駁回申請</button>
                        {safeDepartments.find(d => d.id === selectedAppDetail.deptId)?.managerId === currentEmp?.id && selectedAppDetail.status === 'pending_manager' && <button onClick={() => { handleApproveApplication(selectedAppDetail, 'manager_approve'); setSelectedAppDetail(null); }} className="flex-1 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-black shadow-md transition text-sm">主管核准送出</button>}
                        {isHRAdmin && selectedAppDetail.status === 'pending_hr' && <button onClick={() => { handleApproveApplication(selectedAppDetail, 'hr_approve'); setSelectedAppDetail(null); }} className="flex-1 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-black shadow-md transition text-sm">HR 覆核歸檔</button>}
                        {isHRAdmin && selectedAppDetail.status === 'pending_manager' && <button onClick={() => { handleApproveApplication(selectedAppDetail, 'hr_approve'); setSelectedAppDetail(null); }} className="flex-1 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-black shadow-md transition text-sm" title="無視主管流程，強制通過">天神覆核 (強制通過)</button>}
                      </div>
                    </div>
                  )}
                </div>
